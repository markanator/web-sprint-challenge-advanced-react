...
1. Explain how to build stateful class components.
  - when creating components, you add a ``` state={}``` to it and/or in the Constructor function
  - you can then run API calls or to edit the state

2. Describe the different phases of the component lifecycle.
   - There are three phases to a component's lifecycle.
   - 1. Render Phase
       - No side effects, can be paused/aborted/restarted by React
       - During Mounting... Constructor gets called
       - During Mounting && Updates... Renders
   - 2. Pre-Commit Phase
       - Can read the DOM
   - 3. Commit Phase
       - Work with DOM, Side Effects, updates
       - During Mounting.. gives us ComponentDidMount method
       - During Updating.. gives us ComponentDidUpdate method
       - During Unmounting.. gives us ComponentWillUnmount method

3. Demonstrate an understanding of class component lifecycle methods.
  ```js
  ComponentDidMount(){
    axios.get('API_URL)
    .then(this.setState({...userInfo, [e.t.n]: e.t.v}))
    .catch(err=>log(err))
   }
  ```

4. Define stateful logic.
   - data we import to show the user, that has the ability to change.

5. Describe how to test a React component with React Testing Library.
   - Arrange, Act, Assert
   - We arrange our code so it can be tested (importing component to test, testing libraries, etc)
   - We call a method or func that returns a result that we are going to test...
   - we finally assert if the expected return matched the actual return...
